...
**Object key ordering:** Objects are unordered collections with a **canonical enumeration order**: wherever keys are enumerated — `.keys()`, `.values()`, `.iter()`, the iteration underlying `.map_values()`/`.map_keys()`/`.map_entries()`/`.filter_entries()`, and JSON serialization (`.format_json()`, `.string()`, output document encoding) — keys are walked in **ascending order by Unicode codepoint**. This makes every order-observable operation deterministic and identical across implementations (and matches V1's serialization order). Insertion order is *not* preserved and never observable. Object **equality** compares keys and values regardless of order.
