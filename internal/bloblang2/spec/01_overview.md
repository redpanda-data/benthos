...
**Reserved function names:** `deleted`, `throw`, `void` — these parse as regular function calls but have special semantics (see Sections 8.4, 9.2, 12.3, and 13.1). Like keywords, reserved function names cannot be used as identifiers — they cannot be variable names, map names, or parameter names. They remain valid as field names (e.g., `input.deleted`, `output.throw`, `input.void`) since field access uses the broader `word` pattern.
